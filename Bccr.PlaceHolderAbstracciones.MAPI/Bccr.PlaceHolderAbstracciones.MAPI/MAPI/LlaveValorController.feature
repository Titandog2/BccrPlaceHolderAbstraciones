...
Scenario: Obtener todos los valores en el diccionario de datos
   ...
   Then retorna @Task<ActionResult<IEnumerable<LlaveValor>>>
	| Llave | Valor      |
	| 1     | Corrales   | 
	| 2     | Agüero     | 
	| 3     | Esquivel   | 
...
Scenario: Obtener un valor por llave
   ...
   Then  retorna @Task<ActionResult<string>> Corrales
...
Scenario: Editar un valor
   ...
   Then se edita el valor y se retorna @Task<ActionResult<int>> 2   
...
Scenario: Eliminar un valor
   ...
   Then retorna @Task<ActionResult<int>> 3

Scenario: Eliminar un valor da error cuando la llave no se encuentra en el diccionario
   Given se desea eliminar un valor
